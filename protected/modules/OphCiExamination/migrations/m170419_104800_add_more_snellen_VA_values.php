...
class m170419_104800_add_more_snellen_VA_values extends OEMigration
{
    public function up()
    {
        $unit = $this->dbConnection->createCommand()
        ->select('id')
        ->from('ophciexamination_visual_acuity_unit')
        ->where('name = :name', [':name' => 'Snellen Metre'])
        ->queryRow();

        if (!$unit) {
            echo "Error: 'Snellen Metre' not found. Aborting.\n";
            return;
        }
